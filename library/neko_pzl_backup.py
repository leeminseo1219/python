...
def set_neko():
    for x in range(8):
        neko[0][x] = random.randint(0, difficulty) #블럭을 생성(0번, 1~6:일반블럭)
...
def game_main():
    ...
    if index == 0:  # 타이틀 로고
        ...
        if 24 <= mouse_x and mouse_x < 24 + 72 * 8 and 24 <= mouse_y and mouse_y < 24 + 72 * 10:
            cursor_x = int((mouse_x - 24) / 72) #칸 수 ㅏ만큼 입력 0~7
            cursor_y = int((mouse_y - 24) / 72) #칸 수 ㅏ만큼 입력 0~9
            if mouse_c == 1:
                mouse_c = 0
                set_neko()
...
cvs = tkinter.Canvas(root, width=912, height=768) #1044,896
cvs.pack()

bg = tkinter.PhotoImage(file="C:/Users/my/Desktop/cemester/aa/python/library/neko_bg.png")
...
cvs.create_image(456,384, image=bg) #522,448
game_main()
root.mainloop()
